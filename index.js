...
function checkValidator(optionValue, schemeValue, paths) {
    let schemeValidator;
    if (kindOf(schemeValue) === 'function') {
        schemeValidator = schemeValue;
    } else if (schemeValue.___validator__) {
    ...
    if (kindOf(schemeValidator) === 'function') {
        ...
        if (result !== true) {
            throw new TypeError(
                `The scheme for '${paths.join('.')}' validator function require return true, but got '${result}'`,
            );
        }
    }
}
